...
export function tryFixingPostLengthByRemovingExplore(text) {
...
export function tryFixingPostLengthByRemovingPoolName(text) {
...
export async function tryFixingPostLengthByRemovingFeaturing(launch) {
...
export async function tryFixingPostLengthByRemovingMax(launch) {
    // Generate hashtags from the coin symbols and ensure they are unique
    const hashtags = Array.from(new Set(launch.coins.map((coin) => `#${coin.symbol.replace(/[\W_]+/g, '')}`))).join(' ');
    // Construct the tweet text with organized sections
