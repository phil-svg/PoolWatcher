...
export async function postOnX(tweetText: string): Promise<string | null> {
  try {
    ...
    if ((error as any).data.detail.includes('You are not allowed to create a Tweet with duplicate content.')) {
